...
def main(num_geometries_in_line):
    for i in [0, num_geometries_in_line-1]: # Only at the corners
        for j in [0, num_geometries_in_line-1]: # Only at the corners
            # Adjust the x and y coordinates based on the length of the building
            x_offset = i * (length_member + (space_between_members*100)) # Adjust the offset as needed
            y_offset = j * (length_member + (space_between_members*100)) # Adjust the offset as needed

            # Create centers for A, B, and C
            centers_A = [
                [x_offset, y_offset + (length / 2), 0],
                [x_offset, y_offset + (length) + (length / 2), 0],
                [x_offset, y_offset + (2 * length) + (length / 2), 0],
                [x_offset, y_offset + (4 * length) + (length / 2), 0],
                [x_offset, y_offset + center_A4[1] + length, 0],
                [x_offset, y_offset + center_A4[1] + 2 * length, 0]
            ]

            centers_B = [
                [x_offset + (length / 2) + (width / 2), y_offset + (center_A4[1] - length), 0],
                [x_offset - (length / 2) - (width / 2), y_offset + (center_A4[1] - length), 0],
                [x_offset + (length / 2) + (width / 2), y_offset + (center_A4[1] - length) - length, (length_member / 2) + (width / 2)],
                [x_offset + (length / 2) + (width / 2), y_offset + (center_A4[1] - length) + length, (length_member / 2) + (width / 2)],
                [x_offset - (length / 2) - (width / 2), y_offset + (center_A4[1] - length) - length, -((length_member / 2) + (width / 2))],
                [x_offset - (length / 2) - (width / 2), y_offset + (center_A4[1] - length) + length, -((length_member / 2) + (width / 2))],
                [x_offset + (length / 2) + (width / 2), y_offset + (center_A4[1] - length) - length, -((length_member / 2) + (width / 2))],
            ...
            centers_C = [
                [x_offset, y_offset +(center_A4[1] - length), width],
                [x_offset, y_offset +(center_A4[1] - length), -width],
                [x_offset + length, y_offset + (center_A4[1] - length) + (1.5 * length + length_member / 2), width],
                [x_offset + length, y_offset + (center_A4[1] - length) - (1.5 * length + length_member / 2), -width],
                [x_offset + length, y_offset +(center_A4[1] - length) - (1.5 * length + length_member / 2), width],
                [x_offset + length, y_offset +(center_A4[1] - length) + (1.5 * length + length_member / 2), -width],
                [x_offset - length, y_offset +(center_A4[1] - length) + (1.5 * length + length_member / 2), width],
